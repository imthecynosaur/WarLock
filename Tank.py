...
class Tank:
    ...
    def move(self, destination_x, destination_y):
        teta = math.atan((destination_y - self.rect.centery) /
                         (destination_x - self.rect.centerx))

        if (abs((destination_y - self.rect.centery)) < 4 and abs(destination_x - self.rect.centerx) < 4):
            return
        else:
            if ((destination_x - self.rect.centerx) > 0 or (destination_y - self.rect.centery) > 0):
                self.rect.centerx += Tank.velocity * math.cos(teta)
                self.rect.centery += Tank.velocity * math.sin(teta)
